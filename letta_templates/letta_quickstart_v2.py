...
def create_personalized_agent_v2(
    name: str = "emma_assistant",
    client = None,
    use_claude: bool = False,
    overwrite: bool = False,
    ...
    """Create a personalized agent with V2 memory structure"""
    if client is None:
        client = create_letta_client()
    ...
    # Create configs first (keep same as original)
    llm_config = LLMConfig(
        model="gpt-4o-mini",
        model_endpoint_type="openai",
        model_endpoint="https://api.openai.com/v1",
            ...
            client.create_block(
                ...
                value=json.dumps({
                    "lore": {
                        ...
                        "current_season": "Summer Festival",
                        "major_events": [
                            {
                                "name": "Summer Festival",
                                "description": "Annual celebration with special merchants and events",
                                "status": "ongoing"
                            }
                        ],
                        "factions": [
                            {
                                ...
                                "description": "Organization of traders and craftspeople",
                    ...
                    "announcements": [
                        {
                            ...
                            "content": "The Summer Festival begins today! Visit special merchants.",
                            "timestamp": "2024-03-20T10:00:00Z"
                        }
                    ],
                    "gossip": [
                        {
                            "content": "Rumor has it there's treasure hidden in the garden",
    ...
    # Add task queue tools
    def task_queue_push(task_description: str) -> str:
        """Add a new task to the queue"""
        tasks = json.loads(memory.get_block("tasks").value)
        tasks["queue"].append({
            "description": task_description,
            "created_at": datetime.now().isoformat()
        })
        memory.get_block("tasks").value = json.dumps(tasks)
        return f"Added task: {task_description}"

    def task_queue_pop() -> Optional[str]:
    ...
    Memory Management:
    - Use task_queue_push to add new tasks
    - Use task_queue_pop to get and complete tasks
    - Check global block for world lore and announcements
    - Update status block when your state changes
    - Record social interactions in shared_social
    - Keep participant information current
    """
    ...
    # Keep same tool registration and verification
    if with_custom_tools:
        print("\nRegistering tools:")
        for name, tool_func in TOOL_REGISTRY.items():
            try:
...
def main():
    ...
    try:
        ...
        # Use V2 if flag is set
        agent = (create_personalized_agent_v2 if args.v2 else create_personalized_agent)(
            name=args.name,
            client=client,
